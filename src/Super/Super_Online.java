package Super;

import java.sql.SQLException;
import java.util.Scanner;

public class Super_Online {
	

	static Scanner in = new Scanner(System.in);
	static Sentencias st= new Sentencias();

	public static void main(String[] args) throws SQLException {

		int opcion = 1;
		Conection c = new Conection();
		c.connect();
	

		System.out.println("========================================================================");
		System.out.println("=                 SISTEMA DE GESTION DE SUPER-ON-LINE                  =");
		System.out.println("========================================================================");
		System.out.println();
		System.out.println();


		while (opcion != 0) {
			mostrarMenuInventario();
			System.out.println("Opción? ");
			opcion = leerOpcion(7); // hay 6 acciones principales sobre el inventario + terminar la aplicación
			switch(opcion) {
			case 0:	System.out.println("Termina la aplicación");
					System.exit(0);
			case 1: Inventario.cargarProductos();
					pause();
					break; // cargar productos
			case 2: Inventario.mostrarProductos();
					pause();
					break;
			case 3:     //actualizar la cantidad de un producto en almancén
				Producto p;
				while (opcion!=0) {
					mostrarMenuActualizarCantidads();
					System.out.println("Opción? ");
					opcion = leerOpcion(6); // 5 clases de productos y salida del menú
					if (opcion!=0) {
						modificarProducto(opcion);
						System.out.println("Producto actualizado");
						System.out.println();
						System.out.println();
						pause();
					}
				} opcion=1; break;
			case 4: 	//añadir un nuevo producto al almancén
				while (opcion!=0) {
					mostrarMenuAddNuevoProducto();
					System.out.println("Opción? ");
					opcion = leerOpcion(6); // 5 clases de productos y salida del menú
					if (opcion!=0) {
						Producto P = nuevoProducto(opcion);
						Inventario.addNuevoProducto(P);
						System.out.println("Producto añadido, número "+Inventario.tamano());
						Inventario.getProducto(Inventario.tamano()).imprimir(); //imprime el último producto incluido
						System.out.println();
						System.out.println();
						pause();
					}
				} opcion=1; break;
			case 5: Inventario.mostrarProductosEnviables();
					pause();
					break;
			case 6: Inventario.guardarProductos();
					pause();
					break;
			}
		}
	}

	public static void mostrarMenuInventario() {// ver productos del super ordenados
		System.out.println("====================================================================");
		System.out.println("=            MENU PRINCIPAL - GESTIÓN DE INVENTARIO                =");
		System.out.println("====================================================================");
		System.out.println("            tecla:1   - Cargar productos");
		System.out.println("            tecla:2   - Mostrar inventario");
		System.out.println("            tecla:3   - Actualizar existencias");
		System.out.println("            tecla:4   - Añadir nuevo producto");
		System.out.println("            tecla:5   - Mostrar productos enviables");
		System.out.println("            tecla:6   - Guardar inventario");
		System.out.println("            tecla:0   - Terminar");
		System.out.println("====================================================================");
	}


	public static void mostrarMenuAddNuevoProducto() {// ver productos y escoger uno
		System.out.println("====================================================");
		System.out.println("=   Elige el tipo de producto que quieres añadir:  =");
		System.out.println("====================================================");
		System.out.println("         tecla:1 - Lacteo");
		System.out.println("         tecla:2 - Frutas u hortalizas");
		System.out.println("         tecla:3 - Bebidas");
		System.out.println("         tecla:4 - Herramientas");
		System.out.println("         tecla:5 - Otros");
		System.out.println("         tecla:0 - Cancelar");
		System.out.println("====================================================");
	}
	
	public static void mostrarMenuActualizarCantidads() {// ver productos y escoger uno
		System.out.println("====================================================");
		System.out.println("=   Elige el tipo de producto que quieres modificar:  =");
		System.out.println("====================================================");
		System.out.println("         tecla:1 - Lacteo");
		System.out.println("         tecla:2 - Frutas u hortalizas");
		System.out.println("         tecla:3 - Bebidas");
		System.out.println("         tecla:4 - Herramientas");
		System.out.println("         tecla:5 - Otros");
		System.out.println("         tecla:0 - Cancelar");
		System.out.println("====================================================");
	}

	public static Producto nuevoProducto (int n) {
		Scanner es = new Scanner (System.in);
		Producto P;
		switch(n) {
			case 1: P = new Lacteo(es,st);break;
			case 2: P = new FrutaHortaliza(es,st);break;
			case 3: P = new Bebida(es,st);break;
			case 4: P = new Herramienta(es,st);break;
			default: P = new Otros(es,st);break;
		}
		return P;
	}
	
	public static void modificarProducto (int n) {
		System.out.println("Introduzca la id del producto que quieras actualizar");
		Scanner sc = new Scanner (System.in);
		int id = sc.nextInt();
		System.out.println("introduzca la cantidad");
		int cantidad = sc.nextInt();
		
		switch(n) {
			case 1:st.actualizarLacteo(cantidad,id);break;
			case 2:st.actualizarFrutaHortaliza(cantidad, id);;break;
			case 3: st.actualizarBebida(cantidad, id);;break;
			case 4:st.actualizarHerramienta(cantidad, id); ;break;
			default:st.actualizarOtro(cantidad, id);;break;
		}
		
	}

	private static void pause() {
		System.out.println("(pulsa 0 para continuar...");
		leerOpcion(1);
	}


	private static int leerOpcion(int max) {
		Scanner sc = new Scanner(System.in);
		boolean terminar = false;
		int n = 0;
		while (!terminar) {
			try {
				n = in.nextInt();
				in.nextLine();
				if (n>=max || n<0) {
					throw new Exception();
				}
				terminar = true;
			} catch (Exception e) {
				System.out.println("Opción incorrecta! elije de nuevo");
				sc.nextLine();
				sc.next();
			}
		}
		return n;
	}
}